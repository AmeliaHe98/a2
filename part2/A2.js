 ...
 */
 ...
 var mode = Part.SHAKE; // current mode
// CHECK WEBGL VERSION
...
for (let i = 0; i < 3; ++i) {
  ...
  // FLOOR WITH PATTERN
  floorTextures[i] = (new THREE.TextureLoader().load('images/IMG_20180708_170622.jpg'));
...
var bunnyPosition = {  type: 'v3',  value: new THREE.Vector3(-0.5,-0.6,0.0)};
var lightPosition = {  type: 'v3',  value: new THREE.Vector3(0, 0, 0)};
var eggPosition = {  type: 'v3',  value: new THREE.Vector3(0.0, 0.3, -5.0)};
var discoPosition = {type: 'v3',value: new THREE.Vector3(0, 10, 0)};
var discoRotationAngle = {type: 'f',value: 0.0};
var headRotationSpeed = 0.001;
var armRotationAngle = {type: 'f',value: 0.0};
var armRotationSpeed = 0.001;
var leftLegRotationAngle = {type: 'f',value: 0.0};
var rightLegRotationAngle = {type: 'f', value: 0.0};
...
var movingArmadilloMaterial = new THREE.ShaderMaterial({
  uniforms: {
    rot_angle: rot_angle,
  ...
  'glsl/shake_bunny.fs.glsl',
    ...
    'glsl/laser.fs.glsl',
      ...
      'glsl/discoball.fs.glsl',
        'glsl/moving_armadillo.vs.glsl',
        'glsl/moving_armadillo.fs.glsl'
  ...
  shakeBunnyMaterial.fragmentShader = shaders['glsl/shake_bunny.fs.glsl'];
    ...
    leftLaserMaterial.fragmentShader = shaders['glsl/laser.fs.glsl'];
        ...
        rightLaserMaterial.fragmentShader = shaders['glsl/laser.fs.glsl'];
          ...
          movingArmadilloMaterial.vertexShader = shaders['glsl/moving_armadillo.vs.glsl'];
          movingArmadilloMaterial.fragmentShader = shaders['glsl/moving_armadillo.fs.glsl'];
...
//---------------------------
loadOBJ(Part.LASERS, 'obj/armadillo.obj', movingArmadilloMaterial, 1, 0, 0, 0, 0, 0, 0); // Armadillo
...
loadOBJ(Part.SHAKE, 'obj/armadillo.obj', movingArmadilloMaterial, 1, 0, 1, 0, 0, 0, 0); // Armadillo

var discoGeometry = new THREE.SphereGeometry(1, 32, 32);
var discoball = new THREE.Mesh(discoGeometry, discoMaterial);
scenes[Part.SHAKE].add(discoball);

//----------------------------
// (D) -- LASERS SCENE OBJECTS
// WORK HERE FOR PART 1.B
//---------------------------
...
shakeBunnyMaterial.needsUpdate =true;// Tells three.js that some uniforms might have changed
	movingArmadilloMaterial.needsUpdate = true;
	discoMaterial.needsUpdate = true;

	armRotationAngle.value = (Date.now() * armRotationSpeed)%50;
	rot_angle.value = (Date.now()* headRotationSpeed)%50;
	discoRotationAngle.value = (Date.now()* 0.001)%100;
}
